...
export function Home() {
  ...
  async function handleAddFavorites(dishId) {
    try {
      ...
      const isFavorite = dishesFavorites.filter((item) => item.id === dishId)
        .length;

      if (isFavorite) {
        await api.delete(`favoritos/${dishId}`);
  ...
  useEffect(() => {
    async function searchDishes() {
      ...
      const response = await api.get(
        `/dishes?dish=${search}&ingredients=${search}`
      );
  ...
  return (
    <Container>
      <HeaderUsers onChange={(e) => setSearch(e.target.value)} />

      <Content>
        <About />
        {isLoading ? (
          <div className="loader">
            <ThreeCircles color="#126b37" width="100" height="100" />
          </div>
        ) : (
          <>
