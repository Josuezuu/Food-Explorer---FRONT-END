...
export function Payment() {
  ...
  const [isScreenDesktop, setIsScreeDesktop] = useState(
  ...
  async function handleNewOrder() {
    try {
      setIsLoading(true);
      const response = await api.post("/carrinho", { user_id: user.id });
      ...
      isFinish(false);
      return toast.error(
        "Você está deslogado, faça o login novamente para navegar.",
        {
          position: toast.POSITION.TOP_RIGHT,
        }
      );
    ...
    <Container>
      ...
      {isLoading ? (
        <div className="loader">
          <Circles color="#126b37" width="100" height="100" />
        </div>
      ) : (
        <Content>
          {isScreenDesktop && (
          ...
          <Section title="Pagamento">
            <main>
              ...
              {buttonPix ? (
                <img src={imagePix} alt="imagem do código pix" />
              ) : (
                <div>
                  {whenFinish ? (
                    <Form>
